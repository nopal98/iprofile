        ...
        <div id="home">
            ...
            <div class="row">
                <section class="about-me line col-md-12 padding_30 padbot_45">
                    <p class="top_30"><?php echo $user->about_me ?></p>
                </section>

                <?php if (!empty($user->envato) || !empty($user->upwork) || !empty($user->fiverr) || !empty($user->freelancer)): ?>
                    <section class="about-me line col-md-12 padding_30 padbot_45">
                        <div class="section-title bottom_15"><span></span><h2>Online Marketplace</h2></div>
                        <div class="col-md-12 cs5">
                            <?php if (!empty($user->envato)): ?>
                                <a target="_blank" class="btn btn-secondary btn-sm" href="<?php echo $user->envato ?>">Envato</a>
                            <?php endif ?>
                            <?php if (!empty($user->upwork)): ?>
                                <a target="_blank" class="btn btn-secondary btn-sm" href="<?php echo $user->upwork ?>">Upwork</a>
                            <?php endif ?>
                            <?php if (!empty($user->fiverr)): ?>
                                <a target="_blank" class="btn btn-secondary btn-sm" href="<?php echo $user->fiverr ?>">Fiverr</a>
                            <?php endif ?>
                            <?php if (!empty($user->freelancer)): ?>
                                <a target="_blank" class="btn btn-secondary btn-sm" href="<?php echo $user->freelancer ?>">Freelancer</a>
                            <?php endif ?>
                        </div>
                    </section>
                <?php endif ?>
